...
class Student(UserMixin, db.Model):
    ...
    mentor_id = db.Column(db.Integer, db.ForeignKey('mentor.id'))  #actual column
...
class Mentor(UserMixin, db.Model):
    ...
    students = db.relationship('Student', backref = 'mentor')   # do the Student table gets a pseudo column called mentor  
...
def signUp():
    if request.method == "POST":
        ...
        if(password == cpassword):
            new_user = Student(fname=fname, rollno=rollno, password=hashed_password, email=email, mobno = mobno, mentor = mentor)
...
def search_student(mentorid):
    if request.method == "POST":
        ...
        results = Student.query.filter(or_(Student.fname.like(search), Student.mname.like(search), Student.sname.like(search), Student.rollno.like(search))).all()
